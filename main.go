...
var seedstr, omit string

var randCmd = newRandCmd()
...
func main() {
	...
	if err := randCmd.Execute(); err != nil {
		os.Exit(1)
	}
}
...
func newRandCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			case formatBase64:
				fmt.Fprint(stdout, base64.StdEncoding.EncodeToString(bs)+trailingNewline)
			case formatBinary:
				_, err = os.Stdout.Write(bs)
				fatal(err, "failed to write unformatted bytes to stdout")
			case formatPassword:
				fmt.Fprint(stdout, encodePassword(bs)+trailingNewline)
			case formatUUID:
				bs[6] = (bs[6] & 0x0f) | 0x40
				bs[8] = (bs[8] & 0x3f) | 0x80
				fmt.Fprintf(stdout, "%x-%x-%x-%x-%x%v", bs[0:4], bs[4:6], bs[6:8], bs[8:10], bs[10:], trailingNewline)
			default:
				fmt.Fprint(stdout, hex.EncodeToString(bs)+trailingNewline)
			}
		},
